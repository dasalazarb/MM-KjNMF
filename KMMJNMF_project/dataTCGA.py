...
class dataTCGA:
    ...
    def concatFeatureLabel(perfiles, pathFeatureLabel, prefijo): ## concatena todos los nombres de todas las moleculas en una lista
        ...
        with open(pathFeatureLabel+'/co-mod_best_results_last_run/featureLabel'+prefijo+'.txt', 'w') as filehandle:
            for listitem in featureLabel:
                filehandle.write('%s\n' % listitem)
        return featureLabel
    
    @staticmethod
    def saveBarcode_TCGA(barcodes,pathFeatureLabel): ## guarda los identificadores de los pacientes TCGA
        with open(pathFeatureLabel+'/co-mod_best_results_last_run/barcodes_TCGA.txt', 'w') as filehandle:
